...
function mapBalance(r: BalanceRow): StockBalance {
  return {
    id: r.id,
    ...
    sku: r.sku,
    productNameTh: r.product_name_th,
    ...
    availableToTransfer: r.available_to_transfer,
    version: r.version,
    updatedAt: r.updated_at.toISOString(),
  };
}
function scope(c: AccessContext, facilityId: string | null) {
  if (facilityId && !canAccessFacility(c, facilityId, "READ"))
    throw new AuthorizationError("FORBIDDEN_FACILITY_SCOPE");
  const ids = visibleFacilityIds(c);
  return { all: ids === null, ids: ids ?? [] };
}
export async function listScopedStockBalances(
  c: AccessContext,
  f: StockBalanceFilters,
): Promise<StockBalance[]> {
  const s = scope(c, f.facilityId);
  if (!s.all && s.ids.length === 0) return [];
  const r = await dbPool.query<BalanceRow>(
...
const mapLedger = (r: LedgerRow): StockLedgerLine => ({
  id: r.id,
...
export async function listStockLedger(c: AccessContext, f: StockLedgerFilters) {
  ...
  return r.rows.map(mapLedger);
}
export async function listNetworkStock(
  c: AccessContext,
  facilityId: string | null,
  productId: string | null,
): Promise<NetworkStock[]> {
  const s = scope(c, facilityId);
  if (!s.all && s.ids.length === 0) return [];
  const r = await dbPool.query<{
    ...
    `SELECT p.id product_id,p.sku,p.name_th,p.name_en,f.id facility_id,f.code facility_code,f.name facility_name,COALESCE(sum(b.quantity)FILTER(WHERE b.stock_status NOT IN('IN_TRANSIT','LOST')),0)::text physical,COALESCE(sum(b.quantity)FILTER(WHERE b.stock_status='AVAILABLE'),0)::text available,COALESCE(ss.safety_quantity,0)::text safety,GREATEST(COALESCE(sum(b.quantity)FILTER(WHERE b.stock_status='AVAILABLE'),0)-COALESCE(ss.safety_quantity,0),0)::text transferable,COALESCE(sum(b.quantity)FILTER(WHERE b.stock_status='IN_TRANSIT'),0)::text transit FROM public.stock_balances b JOIN public.facilities f ON f.id=b.facility_id JOIN public.products p ON p.id=b.product_id LEFT JOIN public.safety_stock_rules ss ON ss.facility_id=f.id AND ss.product_id=p.id WHERE f.organization_id=$1 AND p.organization_id=$1 AND($2::boolean OR f.id=ANY($3::bigint[]))AND($4::bigint IS NULL OR f.id=$4)AND($5::bigint IS NULL OR p.id=$5)GROUP BY p.id,p.sku,p.name_th,p.name_en,f.id,f.code,f.name,ss.safety_quantity ORDER BY p.sku,f.code`,
    [c.organization.id, s.all, s.ids, facilityId, productId],
  );
  return r.rows.map((x) => ({
    productId: x.product_id,
    sku: x.sku,
    productNameTh: x.name_th,
    productNameEn: x.name_en,
    facilityId: x.facility_id,
    facilityCode: x.facility_code,
    facilityName: x.facility_name,
    physicalOnHand: x.physical,
    availableQuantity: x.available,
    safetyQuantity: x.safety,
    availableToTransfer: x.transferable,
