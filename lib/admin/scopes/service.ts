...
import type {
  FacilityScopeDto,
  FacilityScopeInput,
  UpdateFacilityScopeInput,
} from "@/lib/admin/scopes/types";
...
type ScopeRow = {
  ...
  scope_type: FacilityScopeDto["scopeType"];
...
export async function createFacilityScope(
  ...
  return withTransaction(async (client) => {
    ...
    const existing = await client.query<ScopeRow>(
      `${selectScope} WHERE s.user_id=$1 AND s.facility_id=$2 FOR UPDATE OF s`,
      [userId, input.facilityId],
    );
    if (
      existing.rows[0] &&
      (!existing.rows[0].valid_until ||
        existing.rows[0].valid_until > new Date())
    ) {
      throw new ConflictError(
        "CONFLICT",
        "An active facility scope already exists.",
      );
    }
    ...
    await writeAuditLog(client, {
      ...
      action: existing.rows[0]
        ? "facility.scope.reactivated"
        : "facility.scope.created",
      entityType: "user_facility_scope",
      entityId: dto.id,
      facilityId: dto.facilityId,
      oldData: existing.rows[0] ? mapScope(existing.rows[0]) : null,
      newData: dto,
...
export async function updateFacilityScope(
  ...
  userId: string,
  scopeId: string,
  input: UpdateFacilityScopeInput,
): Promise<FacilityScopeDto> {
  requireManage(context);
  return withTransaction(async (client) => {
    ...
    if (!result.rows[0])
      throw new ConflictError(
        "VERSION_CONFLICT",
        "The facility scope was updated by another request.",
      );
    const dto = mapScope(result.rows[0]);
    await writeAuditLog(client, {
      organizationId: context.organization.id,
      requestId: requestContext.requestId,
      actorUserId: context.user.id,
      action: "facility.scope.updated",
      entityType: "user_facility_scope",
      entityId: dto.id,
      facilityId: dto.facilityId,
      oldData: mapScope(old.rows[0]),
      newData: dto,
      ipAddress: requestContext.ipAddress,
      userAgent: requestContext.userAgent,
    });
...
export async function revokeFacilityScope(
  ...
  return withTransaction(async (client) => {
    ...
    await writeAuditLog(client, {
      ...
      requestId: requestContext.requestId,
      actorUserId: context.user.id,
      action: "facility.scope.revoked",
      entityType: "user_facility_scope",
      entityId: dto.id,
      facilityId: dto.facilityId,
      oldData: mapScope(old.rows[0]),
      newData: { ...dto, reason: input.reason },
      ipAddress: requestContext.ipAddress,
      userAgent: requestContext.userAgent,
    });
    return dto;
