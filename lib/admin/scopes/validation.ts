...
const scopeTypes = new Set<FacilityScopeType>([
  "READ",
  "OPERATE",
  "APPROVE",
  "ADMIN",
...
function common(body: Record<string, unknown>) {
  const scopeType = requiredText(
    body,
    "scopeType",
  ).toUpperCase() as FacilityScopeType;
  if (!scopeTypes.has(scopeType))
    throw new ValidationError({
      scopeType: "Use READ, OPERATE, APPROVE, or ADMIN.",
    });
...
export function parseCreateFacilityScope(
...
export function parseUpdateFacilityScope(
  body: Record<string, unknown>,
): UpdateFacilityScopeInput {
  rejectUnknownFields(body, [
    "scopeType",
...
export function parseRevokeFacilityScope(body: Record<string, unknown>): {
  version: number;
  reason: string;
} {
