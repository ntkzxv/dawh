...
async function ensureUser(client: PoolClient, userId: string) {
  const user = await client.query(`SELECT 1 FROM public."user" WHERE id=$1`, [
    userId,
  ]);
...
export async function revokeRole(
  ...
  return withTransaction(async (client) => {
    const locked = await client.query<AssignmentRow>(
      `${assignmentSelect} WHERE a.id=$1 AND a.user_id=$2 FOR UPDATE OF a`,
      [assignmentId, userId],
    );
    const before = locked.rows[0];
    if (!before || before.revoked_at)
      throw new NotFoundError("Active role assignment");
    if (before.role_code === "SYSTEM_ADMINISTRATOR") {
      ...
      if ((admins.rows[0]?.count ?? 0) <= 1) {
        throw new ConflictError(
          "CONFLICT",
          "The last active system administrator role cannot be revoked.",
        );
      }
    }
    const result = await client.query<AssignmentRow>(
