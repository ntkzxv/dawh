...
import { createFacility, listVisibleFacilities } from "@/lib/facilities/service";
import { parseCreateFacility, parseFacilityFilters } from "@/lib/facilities/validation";
...
export const GET = apiRoute(async (request) => {
  ...
  const facilities = await listVisibleFacilities(context, page, parseFacilityFilters(new URL(request.url)));
...
export const POST = apiRoute(async (request) => {
  const context = await requireAccess(request, { permission: "admin.facilities.manage" });
  const input = parseCreateFacility(await parseJsonObject(request));
  return jsonOk(request, await createFacility(context, getRequestContext(request), input), 201);
});
