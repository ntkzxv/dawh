...
export async function listVisibleFacilities(
  ...
  filters: FacilityFilters = { search: null, facilityType: null, active: null },
): Promise<Facility[]> {
  const ids = visibleFacilityIds(context);
  const global =
    context.permissions.includes("admin.facilities.read") || ids === null;
  if (!global && ids?.length === 0) return [];
  const r = await dbPool.query<Row>(
    `${select} WHERE f.organization_id=$1 AND ($2::boolean OR f.id=ANY($3::bigint[]))
...
export async function getFacility(
  ...
  const r = await executor.query<Row>(
    `${select} WHERE f.id=$1 AND f.organization_id=$2`,
    [facilityId, context.organization.id],
  );
  if (!r.rows[0]) throw new NotFoundError("Facility");
  if (
    !context.permissions.includes("admin.facilities.read") &&
    !canAccessFacility(context, facilityId, "READ")
  )
...
function duplicate(error: unknown): never {
  if ((error as { code?: string }).code === "23505")
    throw new ConflictError("CONFLICT", "The facility code already exists.");
  throw error;
}
...
export async function updateFacility(
  ...
  try {
    return await withTransaction(async (client) => {
      const before = await getFacility(context, facilityId, client);
      const m = { ...before, ...input };
      const r = await client.query<Row>(
        `WITH c AS (UPDATE public.facilities SET code=$3,name=$4,facility_type=$5,address_line_1=$6,address_line_2=$7,province=$8,district=$9,subdistrict=$10,postal_code=$11,latitude=$12::numeric,longitude=$13::numeric,is_active=$14,version=version+1,updated_by=$15 WHERE id=$1 AND organization_id=$2 AND version=$16 RETURNING *) SELECT c.id,c.organization_id,o.code organization_code,c.code,c.name,c.facility_type,c.address_line_1,c.address_line_2,c.province,c.district,c.subdistrict,c.postal_code,c.latitude::text,c.longitude::text,c.is_active,c.version,c.created_at,c.updated_at FROM c JOIN public.organizations o ON o.id=c.organization_id`,
        [
          facilityId,
          context.organization.id,
          m.code,
      ...
      if (!r.rows[0])
        throw new ConflictError(
          "VERSION_CONFLICT",
          "The facility was updated by another request.",
        );
      const dto = map(r.rows[0]);
      await writeAuditLog(client, {
        organizationId: context.organization.id,
