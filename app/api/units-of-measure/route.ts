...
import { createUnit, listUnits } from "@/lib/units-of-measure/service";
import { parseCreateUnit } from "@/lib/units-of-measure/validation";
...
export const GET = apiRoute(async (r) => {
  ...
  return jsonOk(r, await listUnits(c));
});
export const POST = apiRoute(async (r) => {
  ...
  return jsonOk(
    ...
    await createUnit(
      c,
      getRequestContext(r),
      parseCreateUnit(await parseJsonObject(r)),
    ),
    201,
  );
});
